...
function Round(num) {
	if (num > 0.2 && num <= 0.5) return 1;else if (num < -0.2 && num >= -0.5) return -1;
	// else if(num > 0.5)
	// 	return 1;
	// else if(num < -0.5)
	// 	return -1;
 ...
 */
	...
	var oSlider = node_wrap.getElementsByClassName('mySwiper-slide');
	var oLen = oSlider.length;
	var nodeWidth = node.offsetWidth,
	    nodeHeight = node.offsetHeight;
	var loopXLen = opt.direction.x && (oLen - opt.onceNum + 1) * nodeWidth / opt.onceNum,
	...
	(function () {
		if (opt.pagination !== '' && opt.paginationSelectedClassName !== '') {
			//分页区域
	...
	function jumpToThis(arr, that) {
		var index = Array.prototype.indexOf.call(arr, that);
