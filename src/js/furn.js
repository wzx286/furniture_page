	window.onload = function () {
		...
		const picLen = 9.1 - 0.49 * (picNum - 1);
		...
		Array.prototype.forEach.call(picSlide,function (el,i) {
			...
			i!==0 && (el.style.transform = `translate3d(${picLen + 0.49*(i - 1)}rem,0rem,0rem)` );
		});
		...
		function turn(e) {
			...
			e.stopPropagation();
			for( let pi of picSlide ) {
				pi.index <= index && ( pi.style.transform = `translate3d(${0.49*(pi.index)}rem,0rem,0rem)`) || pi.index!==0 &&( pi.style.transform = `translate3d(${picLen + 0.49*(pi.index - 1)}rem,0rem,0rem)`);
			}
		}
