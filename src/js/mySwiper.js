...
function BindSwiperNode(node,option){//direction,bounce,loop
	...
	opt.bounce = opt.bounce || {x:node.offsetWidth,y:node.offsetHeight};
	...
	opt.direction.x && (node_wrap.style.width = oLen * node.offsetWidth +"px");
	opt.direction.y && (node_wrap.style.height = oLen * node.offsetHeight + "px");
